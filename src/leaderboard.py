...
class LeaderBoard:
    ...
    def __init__(self, round_duration: timedelta = None, expire_delta: timedelta = None, dry_run: bool = True):
        ...
        # Длительность раунда
        self.round_duration = round_duration or timedelta(seconds=duration)
    ...
    def run_update(self, d):
        """Запустить фоновое обновление счётчиков."""
        global duration
        duration = d
        self.round_duration = timedelta(seconds=duration)
        self.last_game = []
        self.round_counter = 0
        self.last_update = time.time()
        def inner():
    ...
    def get_leads(self):
        return self.last_game[:self.visible_leader_board]
